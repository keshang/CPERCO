...
public class BNPP {
	...
	public static double[] BN(double rho, ConnectR connection) throws IOException, REXPMismatchException, REngineException {
        ...
        try {
			...
			IloNumVar zzz = cplex.numVar(0.0,Double.MAX_VALUE);
			...
			for(int i=0; i<n; i++)
			{
				...
				constraint.addTerm(1,yy[i]);
				constraint.addTerm(1,zz[i]);
			...
			for(int i=0; i<n; i++)
			{
				constraint3.addTerm(0.25*C[i]*C[i],zz[i],zz[i]);
			}

			constraint3.addTerm(-1, zzz, zzz);
