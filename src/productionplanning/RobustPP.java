...
public class RobustPP {
    ...
    public static void main(String[] args) throws IOException, REXPMismatchException, REngineException {
        ...
        for (int i = 0; i < Gamma.length; i++) {
            Gamma[i] = (double) i / (Gamma.length - 1) * 1.44;
            //alpha[i] = (double) i / (Gamma.length - 1);
            rho[i] = (double) i / (Gamma.length - 1)* 0.88;
            rsDe[i] = (double) i / (Gamma.length - 1) * 0.5;

            //Gamma[i] = (double) i / (Gamma.length - 1) * mNum;
            //rho[i] = (double) i / (Gamma.length - 1)* Math.sqrt(mNum);
